	...
	Croppic.prototype = {
		...
		crop:function(){
			...
			if(typeof FormData == 'undefined'){
				...
				if (XHR.readyState==4 && XHR.type==200)
					{
		...
		afterCrop: function (data) {
                    ...
                    if(typeof response.type === "undefined"){
                        notifyResponseTimerAlert("Something went wrong. please try again.","error","Error");
                        if (that.options.onError) that.options.onError.call(that,response.message);
                        that.hideLoader();
                        setTimeout( function(){ that.reset(); },2000)
                        return;
                    ...
                    if (response.type == 'error') {
                        notifyResponseTimerAlert(response.message,"error","Error"); 
                        if (that.options.onError) that.options.onError.call(that,response.message);
                        that.hideLoader();
                        setTimeout( function(){ that.reset(); },2000)
                        return;
