...
@section('content')
    @if(!empty($user->portfolio->as_order_key))
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                <ol class="breadcrumb p-l-0 app_stats_breadcrumb">
                    <li><a href="{{ route('home') }}" class="text-info">Home</a></li>
                    <li class="active capitalize">Application Status</li>
                </ol>
                <div class="row application_status_progress">
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <h2 class="page-title m-b-0">{{ $page_title }}</h2>
                        <p class="page-sub-title">{{ $page_sub_title }}</p>
                    </div>
                    <div class="col-lg-8 col-md-8 col-sm-8 col-xs-12 text-center"> 
                        <ul class="wizard-steps application_tab p-l-0" role="tablist">
                            @php
                            ...
                            @endphp
                                <li class="{{ $is_active }}" >
                                    <a id="main_stage_{{ $stage_count }}" class="cpointer" onclick="navigateStages('{{ $key }}');">
                                        <div class="pie_progress" role="progressbar" data-goal="0%" data-barcolor="#41b3f9" data-barsize="5" aria-valuemin="-100" aria-valuemax="100" rel="tooltip" title="{{ $stage['stage_title'] }}">
                                            <div class="pie_progress__content text-muted">{{ $stage_count }}/{{ $total_stages }}</div>
                                        </div>
                                        <div><small class="m-b-0 text-muted">{{ $stage['stage_title'] }}<br><i class="step-count"></i></small></div>
                                        <i class="mdi mdi-alert-outline stage-alert" style="display: none;"></i>
                                    </a>
                                </li>
                            @php
                                $stage_count++;
        ...
        <script type="text/javascript">
           ...
           /** Display toast in J1 Resume Approval Tab**/
            ...
            function showUploadInstruction(doc_req_id){
                var extra_data = { doc_req_id: doc_req_id };
                
                show_popup('modal-lg');
                get_common_ajax("{{ route('upload.document.instruction') }}",extra_data,'modal-lg');
            }
            ...
            function viewUploadHistory(doc_type){
                ...
                show_popup('modal-lg');
                get_common_ajax("{{ route('document.history') }}",extra_data,'modal-lg');
            }
            ...
            <div class="col-md-12">
                <div class="white-box m-t-20">
                    ...
                    <div class="alert alert-warning">
                        <p>Your application status progress will be started shortly.</p>
                    </div>
                </div>
            </div>
        </div>
