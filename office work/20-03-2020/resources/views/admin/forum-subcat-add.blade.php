...
$mode = "Add";
$action = route('forum.subcat.add');


if(!empty($slug)){
    $mode = "Edit";
    $action = route('forum.subcat.edit',$slug);
    ...
    <div class="row bg-title">
        ...
        <div class="col-lg-9 col-sm-8 col-md-8 col-xs-12">
            <ol class="breadcrumb">
                ...
                @if(check_route_access('forum.subcat.list'))
                <li><a href="{{ route('forum.subcat.list') }}">Access Control</a></li>
                @endif
            ...
            <div class="white-box">
                <div class="row">
                    <div class="col-md-6 col-xs-12">
                        <h3 class="box-title m-b-0">{{ $mode }} Forum Sub Category</h3>
                        <!--<p class="text-muted m-b-30">Add New Admin</p>-->
                    </div>
                    <div class="col-md-6 col-xs-12">
                        <div class="pull-right">
                            @if(check_route_access('forum.subcat.list'))
                            <a href="{{ route('forum.subcat.list') }}" class="btn btn-block btn-info"><i class="fa fa-list"></i> Forum Sub Category</a>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="{{ ($mode == "Edit")? 'col-md-6' : 'col-md-12'}}">
                        ...
                        <form method="post" action="{{ $action }}" class="form-horizontal form-validator" enctype="multipart/form-data">
                            ...
                            <div class="form-group {{ $errors->has('parent_cat_id') ? 'has-error' : '' }}">
                                <label class="col-md-12">Category <span class="text-danger">*</span></label>
                                <div class="col-md-12">
                                    <select class="form-control" name="parent_cat_id" id="parent_cat_id" required>
                                        <option value="">Select Forums Category</option>
                                        @foreach($forum_categories as $item)
                                        <option value="{{ $item->id }}" {{ ( $forum->parent_category_id == $item->id || old('parent_cat_id') ==  $item->id) ? 'selected' : '' }}>{{ $item->title }}</option>
                                        @endforeach
                                    </select>
                                    <div class="clearfix"></div>
                                    <div class="help-block with-errors">
                            ...
                            <div class="form-group {{ $errors->has('tags') ? 'has-error' : '' }}">
                                ...
                                <div class="col-md-12">
                                    <input type="text" name="tags" id="tags" placeholder="Topic Tags" class="form-control " >
                                    <div class="clearfix"></div>
                                    <div class="help-block with-errors">
